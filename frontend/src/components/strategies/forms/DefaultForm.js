...
export const DefaultForm = ({strategy, setAddedRepresentations, representations, setCanSubmit}) => {
    useEffect(() => {
        let representationTypes = representations.map(rep => rep.type);
        if (strategy.representationTypes.reduce((p, c) => {return p? representationTypes.includes(c) : false}, true))
            setCanSubmit(true);
    }, []);
