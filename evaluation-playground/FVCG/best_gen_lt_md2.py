...
def evaluation(strategy):
	...
	for cb in codebases:
		...
		with open(results_file_path) as results_file:
			...
			for key in result:
				...
				if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11 and result[key]['maxDepth'] == 2:
	...
	plt.title(f"FVCG Combined metric by Linkage Type")
	plt.savefig(EVALUATION_PATH + f"/best_gen_fvcg_lt_md2.png", format="png", bbox_inches='tight')
	plt.clf()

	# Welch's t-Test
	welchs_ttest(data, 'best_gen_fvcg_lt_md2.txt')
