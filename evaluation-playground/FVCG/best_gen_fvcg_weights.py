...
def evaluation(strategy):
	...
	for i in range(len(df['n'])):
		if (not (df['cb'][i] in tmp_data[df['n'][i]])):
			tmp_data[df['n'][i]][df['cb'][i]] = { 'combined': 1.1, 'controllersWeight': 0, 'entitiesWeight': 0, 'servicesWeight': 0, 'intermediateMethodsWeight': 0 }
		if df['combined'][i] < tmp_data[df['n'][i]][df['cb'][i]]['combined']:
			tmp_data[df['n'][i]][df['cb'][i]]['combined'] = df['combined'][i]
			tmp_data[df['n'][i]][df['cb'][i]]['controllersWeight'] = df['controllersWeight'][i]
			tmp_data[df['n'][i]][df['cb'][i]]['entitiesWeight'] = df['entitiesWeight'][i]
			tmp_data[df['n'][i]][df['cb'][i]]['servicesWeight'] = df['servicesWeight'][i]
			tmp_data[df['n'][i]][df['cb'][i]]['intermediateMethodsWeight'] = df['intermediateMethodsWeight'][i]
	...
	for n in tmp_data:
		for cb in tmp_data[n]:
			...
			tmp_df['controllersWeight'].append(tmp_data[n][cb]['controllersWeight'])
			tmp_df['entitiesWeight'].append(tmp_data[n][cb]['entitiesWeight'])
			tmp_df['servicesWeight'].append(tmp_data[n][cb]['servicesWeight'])
			tmp_df['intermediateMethodsWeight'].append(tmp_data[n][cb]['intermediateMethodsWeight'])
	...
	model = sm.OLS(y, X)
	results = model.fit()
	with open(EVALUATION_PATH + f'/best_gen_fvcg_all_weights.txt', 'w') as file:
	...
	for i in range(len(depend_vars)):
		...
		model = sm.OLS(y, X)
		results = model.fit()
		with open(EVALUATION_PATH + f'/best_gen_fvcg_weights_wo_{depend_vars[i]}.txt', 'w') as file:
	...
	plt.title('Best Combined metric versus controllersWeight')
	plt.xlim(0, 100)
	plt.ylim(0, 1)
	plt.grid(True)
	plt.tight_layout()
	# plt.show()
	plt.savefig(EVALUATION_PATH + f"/best_gen_fvcg_cw.png", format="png", bbox_inches='tight')
	plt.clf()
	...
	plt.ylabel('Combined metric')
	plt.title('Best Combined metric versus entitiesWeight')
	plt.xlim(0, 100)
	plt.ylim(0, 1)
	plt.grid(True)
	plt.tight_layout()
	# plt.show()
	plt.savefig(EVALUATION_PATH + f"/best_gen_fvcg_ew.png", format="png", bbox_inches='tight')
	plt.clf()
	...
	plt.ylabel('Combined metric')
	plt.title('Best Combined metric versus servicesWeight')
	plt.xlim(0, 100)
	plt.ylim(0, 1)
	plt.grid(True)
	plt.tight_layout()
	# plt.show()
	plt.savefig(EVALUATION_PATH + f"/best_gen_fvcg_sw.png", format="png", bbox_inches='tight')
	plt.clf()
	...
	plt.ylabel('Combined metric')
	plt.title('Best Combined metric versus intermediateMethodsWeight')
	plt.xlim(0, 100)
	plt.ylim(0, 1)
	plt.grid(True)
	plt.tight_layout()
	# plt.show()
	plt.savefig(EVALUATION_PATH + f"/best_gen_fvcg_iw.png", format="png", bbox_inches='tight')
	plt.clf()
