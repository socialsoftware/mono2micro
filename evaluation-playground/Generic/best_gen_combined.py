...
def best_general_metrics():
				...
				for key in result:
					if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11:
						...
						if cb in data[result[key]['numberOfEntitiesClusters']][strategy]:
							...
							if data[result[key]['numberOfEntitiesClusters']][strategy][cb] > tc:
								data[result[key]['numberOfEntitiesClusters']][strategy][cb] = tc
						else:
							if max_complexity != 0:
								tc = (1 + (result[key]['complexity'] / max_complexity) + coupling - cohesion) / 3
							data[result[key]['numberOfEntitiesClusters']][strategy][cb] = tc
				...
				for key in result:
					if result[key]['numberClusters'] > 2 and result[key]['numberClusters'] < 11:
						...
						if cb in data[result[key]['numberClusters']][strategy]:
							...
							if data[result[key]['numberClusters']][strategy][cb] > tc:
								data[result[key]['numberClusters']][strategy][cb] = tc
						else:
							if max_complexity != 0:
								tc = (1 + (result[key]['complexity'] / max_complexity) + result[key]['coupling'] - result[key]['cohesion']) / 3
							data[result[key]['numberClusters']][strategy][cb] = tc
	...
	plt.title("Best Combined Values by strategy")
	plt.savefig(EVALUATION_PATH + "/best_general_combined.png", format="png", bbox_inches='tight')

	# Welch's t-Test
	welchs_ttest(data, 'best_general_combined.txt')
