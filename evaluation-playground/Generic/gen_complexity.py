...
def general_metrics():
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11:
						if max_complexity != 0:
							data[result[key]['numberOfEntitiesClusters']][strategy].append(result[key]['complexity'] / max_complexity)
						else:
							data[result[key]['numberOfEntitiesClusters']][strategy].append(0)
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberClusters'] > 2 and result[key]['numberClusters'] < 11:
						if max_complexity != 0:
							data[result[key]['numberClusters']][strategy].append(result[key]['complexity'] / max_complexity)
						else:
							data[result[key]['numberClusters']][strategy].append(0)
	...
	for strategy in ['CA', 'EA', 'FAEA', 'FAMC', 'SA']:
		...
		bp = plt.boxplot(s_data, meanline=False, showmeans=False, medianprops=dict(color='#000000'), whiskerprops=colors[c], flierprops=flierprops, positions= X_axis + i, labels =[k for k in range(3, 11)], patch_artist=True) # , boxprops=colors[c]
		...
		for patch in bp['boxes']:
			patch.set_facecolor(colors[c]['color'])
	...
	plt.title("Uniform Complexity by strategy")
	plt.savefig(EVALUATION_PATH + "/general_complexity.png", format="png", bbox_inches='tight')

	# Welch's t-Test
	welchs_ttest(data, 'general_complexity.txt')
