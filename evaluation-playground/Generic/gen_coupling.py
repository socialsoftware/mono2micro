...
def general_metrics():
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11:
						...
						data[result[key]['numberOfEntitiesClusters']][strategy].append(coupling)
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberClusters'] > 2 and result[key]['numberClusters'] < 11:
						data[result[key]['numberClusters']][strategy].append(result[key]['coupling'])
	...
	plt.title("Coupling by strategy")
	plt.savefig(EVALUATION_PATH + "/general_coupling.png", format="png", bbox_inches='tight')

	# Welch's t-Test
	welchs_ttest(data, 'general_coupling.txt')
