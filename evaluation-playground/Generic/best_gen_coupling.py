...
def best_general_metrics():
				...
				for key in result:
					if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11:
						...
						if cb in data[result[key]['numberOfEntitiesClusters']][strategy]:
							if data[result[key]['numberOfEntitiesClusters']][strategy][cb] > coupling:
								data[result[key]['numberOfEntitiesClusters']][strategy][cb] = coupling
						else:
							data[result[key]['numberOfEntitiesClusters']][strategy][cb] = coupling
				...
				for key in result:
					if result[key]['numberClusters'] > 2 and result[key]['numberClusters'] < 11:
						if cb in data[result[key]['numberClusters']][strategy]:
							if data[result[key]['numberClusters']][strategy][cb] > result[key]['coupling']:
								data[result[key]['numberClusters']][strategy][cb] = result[key]['coupling']
						else:
							data[result[key]['numberClusters']][strategy][cb] = result[key]['coupling']
	...
	plt.title("Best Coupling Values by strategy")
	plt.savefig(EVALUATION_PATH + "/best_general_coupling.png", format="png", bbox_inches='tight')

	# Welch's t-Test
	welchs_ttest(data, 'best_general_coupling.txt')
