...
def best_general_metrics():
				...
				for key in result:
					if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11:
						...
						if cb in data[result[key]['numberOfEntitiesClusters']][strategy]:
							if data[result[key]['numberOfEntitiesClusters']][strategy][cb] < cohesion:
								data[result[key]['numberOfEntitiesClusters']][strategy][cb] = cohesion
						else:
							data[result[key]['numberOfEntitiesClusters']][strategy][cb] = cohesion
				...
				for key in result:
					if result[key]['numberClusters'] > 2 and result[key]['numberClusters'] < 11:
						if cb in data[result[key]['numberClusters']][strategy]:
							if data[result[key]['numberClusters']][strategy][cb] < result[key]['cohesion']:
								data[result[key]['numberClusters']][strategy][cb] = result[key]['cohesion']
						else:
							data[result[key]['numberClusters']][strategy][cb] = result[key]['cohesion']
	...
	plt.title("Best Cohesion Values by strategy")
	plt.savefig(EVALUATION_PATH + "/best_general_cohesion.png", format="png", bbox_inches='tight')

	# Welch's t-Test
	welchs_ttest(data, 'best_general_cohesion.txt')
