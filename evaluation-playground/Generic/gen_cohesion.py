...
def general_metrics():
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11:
						...
						data[result[key]['numberOfEntitiesClusters']][strategy].append(cohesion)
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberClusters'] > 2 and result[key]['numberClusters'] < 11:
						data[result[key]['numberClusters']][strategy].append(result[key]['cohesion'])
	...
	plt.title("Cohesion by strategy")
	plt.savefig(EVALUATION_PATH + "/general_cohesion.png", format="png", bbox_inches='tight')

	# Welch's t-Test
	welchs_ttest(data, 'general_cohesion.txt')
