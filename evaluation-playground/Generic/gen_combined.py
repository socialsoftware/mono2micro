...
def general_metrics():
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberOfEntitiesClusters'] > 2 and result[key]['numberOfEntitiesClusters'] < 11:
						...
						data[result[key]['numberOfEntitiesClusters']][strategy].append(tc)
			...
			with open(result_file_path) as result_file:
				...
				for key in result:
					if result[key]['numberClusters'] > 2 and result[key]['numberClusters'] < 11:
						tc = (1 + result[key]['coupling'] - result[key]['cohesion']) / 3
						if max_complexity != 0:
							tc = (1 + (result[key]['complexity'] / max_complexity) + result[key]['coupling'] - result[key]['cohesion']) / 3
						data[result[key]['numberClusters']][strategy].append(tc)
	...
	plt.title("Combined by strategy")
	plt.savefig(EVALUATION_PATH + "/general_combined.png", format="png", bbox_inches='tight')

	# Welch's t-Test
	welchs_ttest(data, 'general_combined.txt')
