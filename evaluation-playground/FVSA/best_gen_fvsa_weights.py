...
def evaluation(strategy):
	...
	for i in range(len(df['n'])):
		if (not (df['cb'][i] in tmp_data[df['n'][i]])):
			tmp_data[df['n'][i]][df['cb'][i]] = { 'combined': 1.1, 'readMetricWeight': 0, 'writeMetricWeight': 0 }
		if df['combined'][i] < tmp_data[df['n'][i]][df['cb'][i]]['combined']:
			tmp_data[df['n'][i]][df['cb'][i]]['combined'] = df['combined'][i]
			tmp_data[df['n'][i]][df['cb'][i]]['readMetricWeight'] = df['readMetricWeight'][i]
			tmp_data[df['n'][i]][df['cb'][i]]['writeMetricWeight'] = df['writeMetricWeight'][i]
	...
	for n in tmp_data:
		for cb in tmp_data[n]:
			...
			tmp_df['readMetricWeight'].append(tmp_data[n][cb]['readMetricWeight'])
			tmp_df['writeMetricWeight'].append(tmp_data[n][cb]['writeMetricWeight'])
	...
	model = sm.OLS(y, X)
	results = model.fit()
	with open(EVALUATION_PATH + f'/best_gen_FVSA_all_weights.txt', 'w') as file:
	...
	for i in range(len(depend_vars)):
		...
		model = sm.OLS(y, X)
		results = model.fit()
		with open(EVALUATION_PATH + f'/best_gen_fvsa_weights_wo_{depend_vars[i]}.txt', 'w') as file:
	...
	plt.grid(True)
	plt.tight_layout()
	plt.savefig(EVALUATION_PATH + f"/best_gen_fvsa_rw.png", format="png", bbox_inches='tight')
	plt.clf()
	...
	plt.grid(True)
	plt.tight_layout()
	plt.savefig(EVALUATION_PATH + f"/best_gen_fvsa_ww.png", format="png", bbox_inches='tight')
	plt.clf()
