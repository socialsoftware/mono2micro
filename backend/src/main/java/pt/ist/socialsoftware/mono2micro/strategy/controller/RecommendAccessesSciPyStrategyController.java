...
public class RecommendAccessesSciPyStrategyController {
	...
	public ResponseEntity<HttpStatus> analyser(
		...
		try {
			...
			Codebase codebase = FileManager.getInstance().getCodebaseWithFields(
				codebaseName,
				new HashSet<String>() {{
					add("name");
					add("profiles");
			...
			boolean analyserResultFileAlreadyExists = fileManager.analyserResultFileAlreadyExists(codebaseName);
		...
		throws Exception
	{
		...
		HashMap<String, HashMap<String, Set<Short>>> analyserCut = fileManager.getAnalyserCut(
			codebase.getName(),
			filename
		);
		...
		AccessesSource source = (AccessesSource) fileManager.getCodebaseSource(codebase.getName(), ACCESSES);
