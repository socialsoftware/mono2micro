...
public class MoJoCalculator {
    ...
    /* ByteArrayOutputStream as alternatives to files */
    public ByteArrayOutputStream sourceStream;
    public ByteArrayOutputStream targetStream;
    ...
    private int numOfClustersInB = 0; /* number of clusters in B */
    ...
    private void commonPrepUsingByteArrayStreams() {
        ...
        numOfClustersInA = mapClusterTagA.size(); /* number of clusters in A */
        numOfClustersInB = mapClusterTagB.size(); /* number of clusters in B */

        A = new Cluster[numOfClustersInA]; /* create A */
        groupscount = new int[numOfClustersInB]; /* the count of each group, 0 if empty */
        grouptags = new Cluster[numOfClustersInB]; /*
    ...
    private long calculateCost() {
        ...
        totalCost = moves + numOfClustersInA - no_of_nonempty_group;
        return totalCost;
    }

    private long calculateEvoCost() {
        int moves = 0; /* total number of move operations */
        int no_of_nonempty_group = 0; /* number of total noneempty groups */
        long totalCost = 0; /* total cost of MoJo */
        ...
        totalCost = moves + numOfClustersInA - no_of_nonempty_group + extraInA + extraInB*2;
        return totalCost;
    }
    ...
    private void readSourceStream() {
        br_s = new BufferedReader(new InputStreamReader(
                new ByteArrayInputStream(sourceStream.toByteArray())));
        ...
        try {
            for (String line = br_s.readLine(); line != null; line = br_s
                ...
                if (st.countTokens() != 3) {
                    String message = "Incorrect RSF format in " + sourceFile
        ...
        if (extraInA > 0)
            put("Warning: " + extraInA + " objects in " + sourceFile
                    + " were not found in " + targetFile
                    + ". They will be ignored.");
        extraInB = mapObjectClusterInB.keySet().size()
                - numberOfObjectsInA;
        if (extraInB > 0)
            put("Warning: " + extraInB + " objects in " + targetFile
                    + " were not found in " + sourceFile
                    + ". They will be ignored.");
    }
    ...
    private void readSourceRSFfile() {
        ...
        try {
            for (String line = br_s.readLine(); line != null; line = br_s
                ...
                int index = -1;
                String clusterName = strTokList.get(1);
                String objectName = strTokList.get(2);
    ...
    private void readRelationRSFfile() {
        ...
        try {
            for (String line = br_r.readLine(); line != null; line = br_r
                ...
                StringTokenizer st = new StringTokenizer(line);
                if (st.countTokens() != 3) {
                    String message = "Incorrect RSF format in " + relFile
                            + " in the following line:\n" + line;
                    throw new RuntimeException(message);
                }
    ...
    private void readTargetBunchFile() {
        ...
        try {
            for (String line = br_t.readLine(); line != null; line = br_t
                ...
                String objList = line.substring(equalMark + 1, line.length())
                        .trim();
                StringTokenizer st = new StringTokenizer(objList, ",");
                int objNumber = st.countTokens();
    ...
    private void readTargetRSFFile() {
        ...
        try {
            for (String line = br_t.readLine(); line != null; line = br_t
                ...
                StringTokenizer st = new StringTokenizer(line);
                if (st.countTokens() != 3) {
                    String message = "Incorrect RSF format in " + targetFile
                            + " in the following line:\n" + line;
                    throw new RuntimeException(message);
                }
                // Ignore lines that do not start with contain
                ...
                if (first_quote_index == 0
                        && clusterName.indexOf("\"", first_quote_index + 1) == clusterName
                        .length() - 1)
                    clusterName = clusterName.substring(first_quote_index + 1,
                            clusterName.length() - 1);
    ...
    private void readTargetStream() {
        br_t = new BufferedReader(new InputStreamReader(
                new ByteArrayInputStream(targetStream.toByteArray())));
    ...
    private boolean isBunch(String file) {
        ...
        if (extend.equalsIgnoreCase("bunch"))
            return true;
        return false;
    }
