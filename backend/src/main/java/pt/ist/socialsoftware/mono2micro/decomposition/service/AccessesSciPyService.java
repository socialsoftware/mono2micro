...
@Service
public class AccessesSciPyService {
    @Autowired
    ...
    public void snapshotDecomposition(String decompositionName) throws Exception {
        AccessesSciPy decomposition = (AccessesSciPy) accessesDecompositionService.updateOutdatedFunctionalitiesAndMetrics(decompositionName);
        ...
        AccessesSciPy snapshotDecomposition = new AccessesSciPy(decomposition);
        ...
        AccessesSciPyStrategy strategy = (AccessesSciPyStrategy) strategyRepository.findByName(similarity.getStrategy().getName());
